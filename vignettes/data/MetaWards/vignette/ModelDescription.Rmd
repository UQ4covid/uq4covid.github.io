...
(Note the additional ``R` class is needed to avoid a MetaWards error, but does not do anything. Hopefully this will be fixed in a later update of MetaWards.)
...
\begin{align}
    ...
    \mbox{mean incubation period ($T_{\mathrm{inc}}$)}&: (1, 8)\mbox{---Challen et al. (2020)}\\
    $T_E$&: (0.1, 2)\\
    $T_P$&: (1.2, 3)\\
...
* Sample $T_{\mathrm{gen}}$ and $T_{\mathrm{inc}}$.
* Sample $T_E$ s.t. $T_E < T_{\mathrm{gen}}$ and $T_E < T_{\mathrm{inc}}$.
* Set $T_P = T_{\mathrm{inc}} - T_E$.
* Sample $T_{E \to H}$ s.t. $T_{E \to H} > T_P$.
* Set $T_{I_1} = T_{E \to H} - T_P$.
* Sample $T_{I \to R}$ s.t. $T_{I \to R} > T_{I_1}$.
* Set $T_{I_2} = T_{I \to R} - T_{I_1}$.
* **Sample $T_H^j$ from log-linear stuff.**
...
```{r, fig.width = 10, fig.height = 8, echo = FALSE}
p1 <- data.frame(
  Lower = sapply(c(0.1, 1.2), function(x) rexp(1000, rate = 1 / x)) %>%
      apply(1, sum)
  ) %>%
  cbind(
      Upper = sapply(c(2, 3), function(x) rexp(1000, rate = 1 / x)) %>%
          apply(1, sum)
  ) %>%
  gather(bound, value) %>%
  ggplot(aes(x = value, y = ..density..)) +
