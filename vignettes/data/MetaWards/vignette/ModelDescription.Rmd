...
user <- "# Normal status" %>%
  ...
  c("static_play_at_home[\"age2\"] = 1.0")
...
\end{align*}
Here $\beta^{kj}$ is the transmission rate from class $k$ to class $j$ and $N_k$ is the total number of individuals in class $k$. The parameter $\beta_{A \to GP}$ scales the force-of-infection from the $A$ class relative to the other infectious classes.
...
**NEED EXPLANATION OF THE CDC APPROACH HERE**
...
parRanges <- data.frame(
    parameter = c("R0", "TE", "TP", "TI1", "TI2", 
                  "betaAGP", "lock_1_restrict", "lock_2_release"),
...
Once we have generated design points in this way, we need to transform from the *input* space to the *disease* space for MetaWards. A `convertInputToDisease()` R function is given below and can be found in the `R_tools/dataTools.R` script file. This requires an `input` data frame, with columns denoting each *input* parameter (`R0`, `TE`, `TP`, `TI1`, `TI2`, `alphaEP`, `alphaI1H`, `alphaI1D`, `alphaHD`, `eta`, `alphaTH`, `etaTH`, `betaAGP`, `lock_1_restrict`, `lock_2_release`) and rows corresponding to each input point, a number of `repeats` and a column of unique identifiers (`output`).
...
```{r, echo = FALSE, message = messageFlag, warning = warningFlag, eval = TRUE}
tempdir <- getwd()
setwd("..")
