...
* $q^j_{EA} = p^j_{EA}\left(1 - e^{-\gamma_{E}}\right)$ where $\gamma_E = \frac{1}{T_E}$ with $T_E$ the mean incubation period;
...
* $q^j_{EI} = \left(1 - p^j_{EA}\right)\left(1 - e^{-\gamma_{E}}\right)$;
* $q^j_{AR} = 1 - e^{-\gamma_I}$ where $\gamma_I = \frac{1}{T_I}$ with $T_I$ the mean infectious period;
* $q^j_{IH} = p^j_{IH}\left(1 - e^{-\gamma_{I}}\right)$;
* $q^j_{IR} = p^j_{IR}\left(1 - e^{-\gamma_{I}}\right)$;
* $q^j_{ID} = \left(1 - p^j_{IH} - p^j_{IR}\right)\left(1 - e^{-\gamma_{I}}\right)$;
* Similarly to before, we model: $\log\left(p^j_{IH/R}\right) = \alpha_{IH/R} + \eta_{IH/R} \mbox{age}_j$, where $\eta_{IH/R} > 0$ and $\alpha_{IH/R} < -\eta_{IH/R} \max\left(\mbox{age}_j\right)$ to ensure than $0 < p^j_{IH/R} \leq 1$.
* $q^j_{HC} = p^j_{HC}\left(1 - e^{-\gamma_{H}}\right)$ where $\gamma_{H} = \frac{1}{T_H}$ with $T_H$ the mean length of a hospital stay;
* $q^j_{HR} = p^j_{HR}\left(1 - e^{-\gamma_{H}}\right)$;
* $q^j_{HD} = \left(1 - p^j_{HC} - p^j_{HR}\right)\left(1 - e^{-\gamma_{H}}\right)$;
* Similarly to before, we model: $\log\left(p^j_{HC/R}\right) = \alpha_{HC/R} + \eta_{HC/R} \mbox{age}_j$, where $\eta_{HC/R} > 0$ and $\alpha_{HC/R} < -\eta_{HC/R} \max\left(\mbox{age}_j\right)$ to ensure than $0 < p^j_{HC/R} \leq 1$. 
* $q^j_{CR} = p^j_{CR}\left(1 - e^{-\gamma_{C}}\right)$ where $\gamma_C = \frac{1}{T_C}$ with $T_C$ is the mean length of time spent in ICU;
* $q^j_{CD} = \left(1 - p^j_{CR}\right)\left(1 - e^{-\gamma_{C}}\right)$;
* Finally, we model: $\log\left(p^j_{CR}\right) = \alpha_{CR} + \eta_{CR} \mbox{age}_j$, where $\eta_{CR} > 0$ and $\alpha_{CR} < -\eta_{CR} \max\left(\mbox{age}\right)$ to ensure than $0 < p^j_{CR} \leq 1$. 
...
\begin{align}
    ...
    \eta_{IR}&: (0, ?)\\
    \alpha_{HC}&: (?, ?)~\mbox{such that}~\alpha_{HC} < -\eta_{HC} \mbox{age}_M\\
    \eta_{HC}&: (0, ?)\\
    \alpha_{HR}&: (?, ?)~\mbox{such that}~\alpha_{HR} < -\eta_{HR} \mbox{age}_M\\
    \eta_{HR}&: (0, ?)\\
    \alpha_{CR}&: (?, ?)~\mbox{such that}~\alpha_{CR} < -\eta_{CR} \mbox{age}_M\\
    \eta_{CR}&: (0, ?)\\
    \beta^S_{A \to GP}&: (0, 1)\\
    \beta^S_{H \to GP}&: (0, 1)\\
    \beta^S_{C \to GP}&: (0, 1).
...
parRanges <- data.frame(
    parameter = c("r_zero", "incubation_time", "infectious_time", "hospital_time",
                  ...
                  "alphaEA", "etaEA", 
                  "alphaIH", "etaIH", "alphaIR", "etaIR",
                  "alphaHC", "etaHC", "alphaHR", "etaHR",
                  "alphaCR", "etaCR",
                  "GP_A", "GP_H", "GP_C"),
    lower = c(2, 4, 2, 4, 4, 0, 0, rep(c("?", 0), 6), rep(0, 3)),
    upper = c(4, 6, 4, 12, 12, 1, 1, rep(c("?", "?"), 6), rep(1, 3)),
...
If required, we have provided a script `R_tools/reconstruct.R`, that provides a function called `reconstruct()` that is written using `Rcpp` and can be used to reconstruct the time-series counts from the incidence data.
...
```{r, code = readLines("../R_tools/reconstruct.R"), eval = FALSE}
```
