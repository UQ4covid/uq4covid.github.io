...
grViz("digraph simpmod {
    ...
    S [group = gp]; E [group = gp]; I [label = 'I@_{1}', group = gp]; I2 [label = 'I@_{2}', group = gp]; 
    ...
    E -> I [label = 'T@_{E}']
    I -> I2 [label = 'T@_{I1}']
    I2 -> RI [weight = 0, label = 'T@_{I2}']
    I -> DI [weight = 0, label = 'T@_{I1}']
    I -> H [label = 'T@_{I1}']
    H -> RH [label = 'T@_{H}']
    H -> DH [weight = 0, label = 'T@_{H}'] 
    RI -> DI [style = 'invis']
    RH -> DH [style = 'invis']
    E -> A [weight = 0]
    A -> RA [label = 'T@_{I1}']
    ...
    subgraph {
      rank = same; St2; RA; I2; RH;
    }
    subgraph {
      rank = same; St3; DI; DH; RI;
    }
...
* $SEI_1I_2R$: symptomatic infections, leading to recovery.
* $SEI_1D$: symptomatic infections, leading to death.
* $SEI_1HR$: symptomatic infections, leading to hospital and then recovery.
* $SEI_1HD$: symptomatic infections, leading to hospital and then death.

Individuals start in the `genpop` demographic. Those that move through the $SEI_1I_2R$ and $SEI_1D$ pathways remain in the `genpop` demographic, whereas those that progress through the `SEAR` pathway move into the `asymp` demographic. Furthermore, some individuals in the `genpop` demographic can be moved to the `hospital` demographic if they follow the `SEI_1HR` and `SEI_1HD` pathways.

We assume that symptomatic individuals in $I_1$ who get very sick either die or get transferred to hospital for care. Individuals who are symptomatic but not severe transition to the $I_2$ class from which they eventually recover.
...
<p style="color:red">NEED TO CHECK IF WORKERS AND PLAYERS SUM TO POPULATION SIZE</p>

<p style="color:red">to_ill_to_move for I1/I2</p>
...
```{js, code = readLines("../model_code/iterator.py"), eval = FALSE}
```

This requires an additional input file containing the dates of the different stages (which are fixed parameters here). This is stored in the `inputs/user_inputs.txt` file:
...
<p style="color:red">DISCUSS INPUTS FILE</p>
