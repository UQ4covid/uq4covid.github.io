...
$$
where $\mathcal{X}$ is the (multidimensional) parameter space for the latent states. Here the latent states correspond to the probabilities of transitions down the different pathways in the model, and $\pi\left(\mathbf{y} \mid \mathbf{x}, \mathbf{n}\right)$ is a binomial probability mass function based on the relevant probabilities $\mathbf{x}$ (here we assume that given $\mathbf{x}$, the data $y_{iH}$ and $y_{iD}$ are independent). To evaluate the integral we used Monte Carlo simulation such that
$$
...
$$
where (with a slight abuse of notation) $x_j \sim \pi\left(\mathbf{x} \mid \alpha_{EP}, \alpha_{I_1D}, \alpha_{I_1H}, \alpha_{HD}, \eta\right)$. Since $\hat{\pi}\left(\mathbf{y} \mid \alpha_{EP}, \alpha_{I_1D}, \alpha_{I_1H}, \alpha_{HD}, \eta, \mathbf{n}\right)$ is positive and unbiased, we can substitute this estimator in place of the true likelihood in a Metropolis-Hastings algorithm, and it will converge to the true posterior in probability. We found $M = 1$ simulation was sufficient, but we had to run long chains to get good mixing.

We repeated this approach for 10 replicate pseudo-data sets derived from the [Verity et al. (2020)](https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(20)30243-7/fulltext) estimates, and also 10 replicate pseudo-data sets derived from the CDC data. The **union** of these posterior samples across all 20 data sets provided our plausible ranges. Full details can be found in the `data/pathways/` folder.
...
tempdir <- getwd()
setwd("..")
system("zip metawards.zip convertDesign.R downloadQuantiles.R runscript.sh model_code/*.py model_code/*.json R_tools/* vignette/ModelDescription.html inputs/*.csv inputs/user_inputs.txt inputs/hospStays.rds inputs/pathways.rds")
...
tempdir <- getwd()
setwd("..")
system("zip metawardsCatalyst.zip *.sh *.sbatch *.R model_code/*.py model_code/*.json R_tools/* vignette/ModelDescription.html inputs/*.csv inputs/user_inputs.txt inputs/hospStays.rds inputs/pathways.rds README.md")
