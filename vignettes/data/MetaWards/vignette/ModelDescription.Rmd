...
**CURRENTLY ONLY WORKS WITH DEVELOPMENT BRANCH OF METAWARDS, BUT SHOULD WORK WHEN RELEASED AS 1.2.0**
...
* $p_{SE}$ driven by $\beta$ parameters, defined by $R_0$ and length of infectious period (see [here](#code));
...
As an example of this latter approach, we will set up a query that sums the **new** hospital cases over the first 100 days in each ward. **Remember**: the database only contains days when some change occurs. For cumulative *incidence* counts this is fine, since the missing data will be zero in each case, so we just need to filter first to remove all time points $> 100$. To set up the query:
