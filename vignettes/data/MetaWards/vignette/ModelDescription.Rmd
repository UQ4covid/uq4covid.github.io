...
demo <- "{"
demo <- c(demo, paste0("\t\"demographics\" : [", paste0(paste0("\"genpop", 1:nrow(ages), "\""), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(paste0("\"asymp", 1:nrow(ages), "\""), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(paste0("\"hospital", 1:nrow(ages), "\""), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(paste0("\"critical", 1:nrow(ages), "\""), collapse = ", "), "],"))
demo <- c(demo, paste0("\t\"work_ratios\" : [", paste0(ages$prop, collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), "],"))
demo <- c(demo, paste0("\t\"play_ratios\" : [", paste0(ages$prop, collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("0.0", nrow(ages)), collapse = ", "), "],"))
demo <- c(demo, paste0("\t\"diseases\" : [", paste0(rep("\"ncov_genpop\"", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("\"ncov_asymp\"", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("\"ncov_hospital\"", nrow(ages)), collapse = ", "), ","))
demo <- c(demo, paste0("\t\t", paste0(rep("\"ncov_critical\"", nrow(ages)), collapse = ", "), "]"))
demo <- c(demo, "}")
...
<p style="color:red">NEED TO CHECK WORK / PLAY RATIOS IN E.G. HOSPITALS AND ICUS</p>
...
<p style="color:red">CURRENTLY MODEL SEEDS IN THE FIRST DEMOGRAPHIC (KIDS) - NEED TO CHANGE THIS IN THE SEED. DISCUSS HOW BEST TO DO THIS.</p>
...
Here `ncov_critical.json` contains:

```{js, code = readLines("../model_code/ncov_critical.json"), eval = FALSE}
```

Notice that individuals in hospital and ICU have `too_ill_to_move = 1.0`, so all individuals are too ill to move.

<p style="color:red">HOW SHOULD WE SCALE `too_ill_to_move` FOR SYMPTOMATICS?</p>
<p style="color:red">HOW DO WE REMOVE INDIVIDUALS FROM THE MODEL ENTIRELY (I.E. AFTER THEY DIE)?</p>

We will discuss these choices in more detail in the subsequent sections. Note that the `progress` parameters are all set to zero here, since all movements will be controlled via the custom `mover` function, and all `beta` parameters are set to one because the scaling of the force-of-infection between the demographics will be controlled by a custom `mixer` function.
...
* $q^j_{SE}$ driven by $\beta_{kj}$ parameters ($k = 1, \dots, J$), defined by $R_0$, the mean length of the infectious period ($1 / \gamma_I$), and the population contact matrix, $C$, between age-classes (see [here](#trans) for details);
* $q^j_{EA} = p_{EA}\left(1 - e^{-\gamma_{E}}\right)$ where $\gamma_E = \frac{1}{T_E}$ with $T_E$ the mean incubation period;
* $q^j_{EI} = \left(1 - p_{EA}\right)\left(1 - e^{-\gamma_{E}}\right)$;
* $q^j_{AR} = 1 - e^{-\gamma_I}$ where $\gamma_I = \frac{1}{T_I}$ with $T_I$ the mean infectious period;
* $q^j_{IH} = p_{IH}\left(1 - e^{-\gamma_{I}}\right)$;
* $q^j_{IR} = p_{IR}\left(1 - e^{-\gamma_{I}}\right)$;
* $q^j_{ID} = \left(1 - p_{IH} - p_{IR}\right)\left(1 - e^{-\gamma_{I}}\right)$;
* $q^j_{HC} = p_{HC}\left(1 - e^{-\gamma_{H}}\right)$ where $\gamma_{H} = \frac{1}{T_H}$ with $T_H$ the mean length of a hospital stay;
* $q^j_{HR} = p_{HR}\left(1 - e^{-\gamma_{H}}\right)$;
* $q^j_{HD} = \left(1 - p_{HC} - p_{HR}\right)\left(1 - e^{-\gamma_{H}}\right)$;
* $q^j_{CR} = p_{CR}\left(1 - e^{-\gamma_{C}}\right)$ where $\gamma_C = \frac{1}{T_C}$ with $T_C$ is the mean length of time spent in ICU;
* $q^j_{CD} = \left(1 - p_{CR}\right)\left(1 - e^{-\gamma_{C}}\right)$.
...
If we ignore the incubation period, and assume the model is $SIR$, then $R_0$ can be derived as the **maximum eigenvalue** of the **next-generation matrix** (NGM), thus:
\begin{align}
...
<p style="color:red">NEED TO CHECK $\nu$ IS A PROBABILITY</p>

<p style="color:red">NEED TO ADD INCUBATION PERIOD TO NGM</p>
...
The **interaction matrix** scales the force-of-infection (FOI) that different demographics have on other demographics. We will use this to capture the effect of each age-class on the FOI for the other age-classes (see [here](#trans)). We also have three further free parameters: $\beta^S_{A \to GP}$, $\beta^S_{H \to GP}$ and $\beta^S_{C \to GP}$ which scale the impact of aymptomatics, hospital cases and critical care cases on the FOI to the general population. We store this information in a file called `mix_pathways.py`. 
...
<p style="color:red">HAVEN'T TOUCHED THIS YET</p>
...
This requires an additional input file containing the dates of the different stages (which are fixed parameters here). This is stored in the `user_inputs.txt` file:
...
<div style="color:red">
Need to discuss this some more. Currently this stores all classes at all time points for each demographic once a ward is infected (so doesn't store any information before the first time point affected). Further options include:

* Only store demographic information once a **demographic** is affected (so if a ward never has individuals in hospital, then no hospital information is recorded).
* Only store time points where something changes.
* Only store a subset of results (we may not need I, E etc. since these can be reconstructed from Iprime, Eprime, R and D for example).

All of these ideas swap storage for additional post-processing requirements.
</div>

We also have a custom `extractor` function, that saves the outputs as a compressed SQL database for each age-class, called `stages*.db.bz2` (where `*` varies by age-class). This is important since the outputs are very large (all classes across all days and all wards, for multiple replicates of each design point). 

The database for a given run contains a single table called `compact`. To store the outputs efficiently, we only return data for time points *after initial infection in a ward*. This also means that wards that are not infected do not return any results. 

This trick **hugely** reduces the size of the output data, but means that we have to do some **post-processing** in order to extract quantities of interest. Since the data are stored as an SQL database, we can either query the database directly, or use some of the tools in e.g. R (see below) to interface with it. So, to clarify, the `stages*.db` databases each contain a table called `compact` with entries:
...
* `R`: current removals on each day (i.e. number of individuals in $R$)
* `D`: current deaths on each day (i.e. number of individuals in $D$)
...
metawards -d model_code/ncov_overall.json -D model_code/demographics.json --mixer model_code/mix_pathways\
...
As described [above](#extractor) each model run produces a file called `stages.db.bz2`, which is a compressed database containing the outputs. In practice, it is time-consuming to unzip and extract all of these outputs for all design points, and extract and collate the various quantities we need for our emulation and history matching process. To this end we have been kindly offered a processing and data storage solution on [JASMIN](https://www.jasmin.ac.uk/). 
...
As a quick example, imagine that we want to extract the **cumulative hospital cases** on say day 100 from R. Here we will need to extract the **new hospital cases** from day 1--100, and then sum them up for each ward. Therefore we need to extract `day`, `ward` and `Hinc` from the `compact` table.
