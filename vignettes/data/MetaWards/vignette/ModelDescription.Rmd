...
**Requires MetaWards `devel` branch install currently**
...
Here the first column is the age demographic and the second is the probability of seeding into that age-demographic. We take the number of deaths observed on each day up to the 14th March 2020, and seed the same number of individuals exactly three weeks earlier. To do this we take a multinomial sample to decide how many individuals are seeded into each age-class, and then we split these across wards as described below. These are stored in the file `inputs/time_seeds.csv`, the first column of which is the date of the seeding, and the second is a baseline for how many individuals are seeded on that day. In practice we multiply these baseline values by a scaling factor `nseedMult`, which is included as a variable input in the model code below.

We take all recorded deaths up to and including the 14th March, recorded at the LAD-level, and for each we seed an infection three weeks before in a random ward that is mapped to the LAD (proportionately to the population size in each ward). Details and instructions can be found in `data/seedDeaths/README.md`. This creates a file `inputs/ward_seeds.csv` that contains the necessary probabilities for ward seeding. This file has the same form as the `age_seeds.csv` file described above, but the first column is the ward ID, the second is a LAD grouping ID, and the third contains the probabilities of seeding. It also creates a file `inputs/time_seeds.csv`, which has a column of dates, a column of LAD grouping IDs and a column for the baseline number of seeds on each date in each LAD.
...
We also require inputs for $R_0$, a parameter $\nu_A$ which scales the force-of-infection between asymptomatics and the general population, and further scaling parameters that we use to model various lockdown measures (described [below](#lockdown)). We also need the scaling parameter for the number of seeds, $n_s$, and a parameter $p_{\text{home/weekend}}$ which scales the number of movements at weekends.
...
Imagine now that we wish to extract the number of asymptomatic infections on each of the first 20 days in each ward. **Remember**: for each ward the database only contains days when some event happens, and only contains *incidence* counts, so we will have collect the data into R, and then run the `reconstruct()` function provided [above](#reconstruct) in each ward. 

> **Note**: the `reconstruct()` function requires that the data are in time order, but do not require time points to be expanded (since no events happen on days that are not present in the data set). For multiple wards the reconstruct function has to run on each ward separately, and hence the code below first orders the data, and then nests it by ward, before running the reconstruct function on each ward separately. It then removes the nesting.
...
## now sort by day, group by ward and run reconstruct()
...
select(output, day, ward, Ainc, A, RA, everything()) %>%
    filter(ward == 459) %>%
    print(n = Inf)
```

At this point we have a set of time-series counts, which we will now expand to fill in the missing ward / day combinations. (This can be done with `complete()`, but we've found it's generally faster by joining to a lookup table as below.)

```{r}
## create lookup to expand ward / time combinations
lookup <- expand.grid(ward = 1:8071, day = 0:20) %>%
    as_tibble()

## joint to lookup, and fill in missing information correctly
asymp <- select(output, day, ward, A) %>%
...
system("zip metawards.zip convertDesign.R runscript.sh model_code/*.py model_code/*.json R_tools/* vignette/ModelDescription.html inputs/*_matrix.csv inputs/user_inputs.txt inputs/hospStays.rds inputs/pathways.rds inputs/*Lookup.csv")
...
system("zip metawardsCatalyst.zip *.sh convertDesign.R JASMIN*/* model_code/*.py model_code/*.json R_tools/* vignette/ModelDescription.html inputs/*.csv inputs/user_inputs.txt inputs/hospStays.rds inputs/pathways.rds inputs/*Lookup.csv README.md")
