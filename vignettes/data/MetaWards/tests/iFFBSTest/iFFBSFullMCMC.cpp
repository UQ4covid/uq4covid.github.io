...
double loglikelihood (double beta, double betaA, arma::mat *pXsum, arma::imat *nXsum, arma::imat *nXcum, arma::ivec *t, double pini, int Npop, arma::ivec *RA, arma::ivec *RI, arma::ivec *DI, arma::ivec *RH, arma::ivec *DH, arma::ivec *P) {
...
void forwardsFilter(int i, double beta, double betaA, arma::imat *Xind, arma::cube *pXind, arma::imat *nXsum, arma::imat *nXcum, arma::mat *pXsum, arma::ivec *t, arma::ivec *DI, arma::ivec *DH, arma::imat *RI, arma::ivec *RH, arma::ivec *RA, arma::ivec *P, int Npop, double pini, arma::vec *tempProb, int print) {
...
arma::mat iFFBS(
    ...
    for(j = 0; j < t.n_elem; j++) {
        Ninf += DI_prime(j) + DH_prime(j) + RA_prime(j) + RI_prime(j) + RH_prime(j);
    }
    int Nsus = Npop - Ninf;
    int Nposs = 10 * Ninf;
    ...
    arma::ivec RA(RA_prime.n_elem);
    RA(0) = RA_prime(0);
    for(i = 1; i < RA_prime.n_elem; i++) RA(i) = RA(i - 1) + RA_prime(i);
    arma::ivec RI(RI_prime.n_elem);
    RI(0) = RI_prime(0);
    for(i = 1; i < RI_prime.n_elem; i++) RI(i) = RI(i - 1) + RI_prime(i);
    arma::ivec RH(RH_prime.n_elem);
    RH(0) = RH_prime(0);
    for(i = 1; i < RH_prime.n_elem; i++) RH(i) = RH(i - 1) + RH_prime(i);
    arma::ivec P(P_prime.n_elem);
    P(0) = P_prime(0);
    for(i = 1; i < P_prime.n_elem; i++) P(i) = P(i - 1) + P_prime(i);
    ...
    while(std::isnan(acc)) {
        ...
        acc = loglikelihood(pars(9), pars(10), &pXsum, &nXsum, &nXcum, &t, pini, Npop, &RA, &RI, &DI, &RH, &DH, &P);
    ...
    for(iter = 0; iter < Niter; iter++) {
        ...
        if(fixPars == 0) {
            ...
            if(k == 0) {  
                ...
                // calculate log-likelihood
                accprop = loglikelihood (parsProp(9), parsProp(10), &pXsum, &nXsum, &nXcum, &t, pini, Npop, &RA, &RI, &DI, &RH, &DH, &P);
...
//        Rprintf("inf\n");
        ...
        while(i < Ninf) {
            ...
            // forwards filter
            forwardsFilter(i, pars(9), pars(10), &Xind, &pXind, &nXsum, &nXcum, &pXsum, &t, &DI, &DH, &RI, &RH, &RA, &P, Npop, pini, &tempProb, print);
...
//        Rprintf("Ninf = %d\n", Ninf);
        ...
        if(fixPars == 0) {
            ...
            output(iter, npars + t.n_elem * 12) = loglikelihood(pars(9), pars(10), &pXsum, &nXsum, &nXcum, &t, pini, Npop, &RA, &RI, &DI, &RH, &DH, &P);
        } else {        
            for(int r = 0; r < t.n_elem; r++) {
                for(int c = 0; c < 12; c++) {
            ...
            output(iter, t.n_elem * 12) = loglikelihood(pars(9), pars(10), &pXsum, &nXsum, &nXcum, &t, pini, Npop, &RA, &RI, &DI, &RH, &DH, &P);
        }   
